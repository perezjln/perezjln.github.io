---
title: Publications
description: "Research publications by Julien Perez in Machine Learning, Deep Learning, Reinforcement Learning, Natural Language Processing, and Dialog Systems"
keywords:
  - "Machine Learning Publications"
  - "Deep Learning Research Papers"
  - "Reinforcement Learning Papers"
  - "NLP Research"
  - "Dialog Systems Papers"
  - "AI Research Publications"
  - "Julien Perez Publications"
cms_exclude: true

# View.
view: citation

# Optional header image (relative to `static/media/` folder).
banner:
  caption: ''
  image: ''
---

## Publications

### 2025

**AKD: Adversarial Knowledge Distillation For Large Language Models Alignment on Coding tasks**  
*Ilyas Oulkadda, Julien Perez*  
CoRR abs/2505.06267 (2025)  
**Keywords:** Large Language Models, Knowledge Distillation, Adversarial Training, Code Generation, LLM Alignment  
[Paper](https://arxiv.org/abs/2505.06267)

### 2024

**Unsupervised Skill Discovery for Robotic Manipulation through Automatic Task Generation**  
*Paul Jansonnie, Bingbing Wu, Julien Perez, Jan Peters*  
Humanoids 2024; also preprint CoRR abs/2410.04855  
**Keywords:** Robotics, Unsupervised Learning, Skill Discovery, Manipulation, Task Generation  
[Paper](https://arxiv.org/abs/2410.04855)

**SLIM: Skill Learning with Multiple Critics**  
*David Emukpere, Bingbing Wu, Julien Perez, Jean‑Michel Renders*  
ICRA 2024; also CoRR abs/2402.00823  
**Keywords:** Reinforcement Learning, Skill Learning, Multi-Critic Networks, Robotics  
[Paper](https://arxiv.org/abs/2402.00823)

**Attention‑Based Cloth Manipulation from Model‑free Topological Representation**  
*Kevin Galassi, Bingbing Wu, Julien Perez, Gianluca Palli, Jean‑Michel Renders*  
ICRA 2024  
**Keywords:** Robotics, Cloth Manipulation, Attention Mechanisms, Topological Representation, Computer Vision  
[Paper](https://dblp.org)

**DiffVersify: a Scalable Approach to Differentiable Pattern Mining with Coverage Regularization**  
*Thibaut Chataing, Julien Perez, Marc Plantevit, Céline Robardet*  
ECML/PKDD (6) 2024  
**Keywords:** Pattern Mining, Differentiable Programming, Deep Learning, Data Mining, Regularization  
[Paper](https://doi.org/10.1007/978-3-031-70365-2_24)

### 2023

**A Hierarchical Scheme for Adapting Learned Quadruped Locomotion**  
*Michel Aractingi, Pierre‑Alexandre Léziart, Thomas Flayols, Julien Perez, Tomi Silander, Philippe Souères*  
Humanoids 2023  
**Keywords:** Robotics, Quadruped Locomotion, Hierarchical Control, Reinforcement Learning, Adaptation  
[Paper](https://dblp.org)

**Safety‑Aware Unsupervised Skill Discovery**  
*Sunin Kim, Jaewoon Kwon, Taeyoon Lee, Younghyo Park, Julien Perez*  
ICRA 2023  
**Keywords:** Reinforcement Learning, Safety, Skill Discovery, Unsupervised Learning, Robotics  
[Paper](https://doi.org/10.1109/ICRA48891.2023.10160985)

**LARG, Language‑based Automatic Reward and Goal Generation**  
*Julien Perez, Denys Proux, Claude Roux, Michael Niemaz*  
CoRR abs/2306.10985 (2023)  
**Keywords:** Natural Language Processing, Reward Learning, Goal Generation, Reinforcement Learning, Language Models  
[Paper](https://arxiv.org/abs/2306.10985)

**Controlling the Solo12 Quadruped Robot with Deep Reinforcement Learning**  
*Michel Aractingi, Pierre‑Alexandre Léziart, Thomas Flayols, Julien Perez, Tomi Silander, Philippe Souères*  
CoRR abs/2309.16683 (2023)  
**Keywords:** Deep Reinforcement Learning, Quadruped Control, Robotics, Neural Networks, Locomotion  
[Paper](https://arxiv.org/abs/2309.16683)

### 2021

**Towards syntax‑aware token embeddings**  
*Diana Nicoleta Popa, Julien Perez, James Henderson, Éric Gaussier*  
Nat. Lang. Eng. 27(6): 691–720 (2021)  
**Keywords:** Natural Language Processing, Token Embeddings, Syntax Analysis, Neural Networks, Language Models  
[Paper](https://dblp.org)

**Globalizing BERT‑based Transformer Architectures for Long Document Summarization**  
*Quentin Grail, Julien Perez, Éric Gaussier*  
EACL 2021: 1792–1810  
**Keywords:** BERT, Transformers, Document Summarization, Natural Language Processing, Long Documents  
[Paper](https://dblp.org)

**Demonstration‑Conditioned Reinforcement Learning for Few‑Shot Imitation**  
*Christopher R. Dance, Julien Perez, Théo Cachet*  
ICML 2021: 2376–2387  
**Keywords:** Reinforcement Learning, Imitation Learning, Few-Shot Learning, Demonstrations, Machine Learning  
[Paper](http://proceedings.mlr.press/v139/dance21a.html)

**Learning Reachable Manifold and Inverse Mapping for a Redundant Robot manipulator**  
*Seungsu Kim, Julien Perez*  
ICRA 2021: 4731–4737  
**Keywords:** Robotics, Manifold Learning, Inverse Kinematics, Robot Manipulation, Machine Learning  
[Paper](https://doi.org/10.1109/ICRA48506.2021.9561589)

### 2020

**Learning Visual Representations with Caption Annotations**  
*Mert Bülent Sariyildiz, Julien Perez, Diane Larlus*  
ECCV (8) 2020: 153–170; also CoRR abs/2008.01392  
**Keywords:** Computer Vision, Visual Representations, Caption Learning, Deep Learning, Multimodal Learning  
[Paper](https://arxiv.org/abs/2008.01392)

### 2019

**Overview of the sixth dialog system technology challenge: DSTC6**  
*Chiori Hori, Julien Perez, et al.*  
Comput. Speech Lang. 55: 1–25 (2019)  
**Keywords:** Dialog Systems, Conversational AI, Natural Language Processing, Speech Recognition, Challenge  
[Paper](https://dblp.org)

**Implicit Discourse Relation Classification with Syntax‑Aware Contextualized Word Representations**  
*Diana Nicoleta Popa, Julien Perez, et al.*  
FLAIRS 2019: 203–208  
**Keywords:** Natural Language Processing, Discourse Analysis, Syntax, Contextualized Embeddings, Classification  
[Paper](https://dblp.org)

**Dialog System Technology Challenge 7**  
*Koichiro Yoshino, Chiori Hori, Julien Perez, et al.*  
CoRR abs/1901.03461 (2019)  
**Keywords:** Dialog Systems, Conversational AI, Natural Language Processing, Technology Challenge, Evaluation  
[Paper](https://arxiv.org/abs/1901.03461)

### 2018

**ReviewQA: a relational aspect‑based opinion reading dataset**  
*Quentin Grail, Julien Perez*  
CoRR abs/1810.12196 (2018)  
**Keywords:** Natural Language Processing, Opinion Mining, Question Answering, Dataset, Sentiment Analysis  
[Paper](https://arxiv.org/abs/1810.12196)

### 2017

**Gated End‑to‑End Memory Networks**  
*Fei Liu, Julien Perez*  
EACL (1) 2017: 1–10  
**Keywords:** Memory Networks, Deep Learning, Natural Language Processing, Gating Mechanisms, Neural Networks  
[Paper](https://aclanthology.org/E17-1001/)

**Dialog state tracking, a machine reading approach using Memory Network**  
*Julien Perez, Fei Liu*  
EACL (1) 2017: 305–314  
**Keywords:** Dialog Systems, State Tracking, Memory Networks, Machine Reading, Natural Language Processing  
[Paper](https://dblp.org)

**A Language‑independent and Compositional Model for Personality Trait Recognition from Short Texts**  
*Fei Liu, Julien Perez, Scott Nowson*  
EACL (1) 2017: 754–764  
**Keywords:** Natural Language Processing, Personality Recognition, Text Analysis, Compositional Models, Psychology  
[Paper](https://dblp.org)

**Hybrid Feature Factored System for Scoring Extracted Passage Relevance in Regulatory Filings**  
*Denys Proux, Claude Roux, Ágnes Sándor, Julien Perez*  
DSMM@SIGMOD 2017: 11:1–11:6  
**Keywords:** Information Retrieval, Regulatory Documents, Feature Engineering, Text Mining, Relevance Scoring  
[Paper](https://dblp.org)

**Non‑Markovian Control with Gated End‑to‑End Memory Policy Networks**  
*Julien Perez, Tomi Silander*  
CoRR abs/1705.10993 (2017)  
**Keywords:** Reinforcement Learning, Non-Markovian Control, Memory Networks, Policy Networks, Control Theory  
[Paper](https://arxiv.org/abs/1705.10993)

### 2016

**Spectral decomposition method of dialog state tracking via collective matrix factorization**  
*Julien Perez*  
Dialogue Discourse 7(3): 34–46 (2016)  
**Keywords:** Dialog Systems, State Tracking, Spectral Methods, Matrix Factorization, Natural Language Processing  
[Paper](https://dblp.org)

**Energy‑aware probing period dimensioning algorithms for mobile WSN‑HEAP**  
*Nouha Sghaier, Julien Perez, Brice Augustin, Abdelhamid Mellouk*  
Int. J. Sens. Networks 21(4): 252–261 (2016)  
**Keywords:** Wireless Sensor Networks, Energy Efficiency, Mobile Networks, Algorithms, Network Optimization  
[Paper](https://dblp.org)

**A Recurrent and Compositional Model for Personality Trait Recognition from Short Texts**  
*Fei Liu, Julien Perez, Scott Nowson*  
PEOPLES@COLING 2016: 20–29  
**Keywords:** Natural Language Processing, Personality Recognition, Recurrent Neural Networks, Text Analysis, Psychology  
[Paper](https://dblp.org)

**XRCE at SemEval‑2016 Task 5: Feedbacked Ensemble Modeling on Syntactico‑Semantic Knowledge for Aspect‑Based Sentiment Analysis**  
*Caroline Brun, Julien Perez, Claude Roux*  
SemEval@NAACL‑HLT 2016: 277–281  
**Keywords:** Sentiment Analysis, Ensemble Methods, Syntactic Analysis, Semantic Analysis, Natural Language Processing  
[Paper](https://dblp.org)

**Dialog state tracking, a machine reading approach using a memory‑enhanced neural network**  
*Julien Perez*  
CoRR abs/1606.04052 (2016)  
**Keywords:** Dialog Systems, State Tracking, Memory Networks, Machine Reading, Neural Networks  
[Paper](https://arxiv.org/abs/1606.04052)

**Spectral decomposition method of dialog state tracking via collective matrix factorization**  
*Julien Perez*  
CoRR abs/1606.05286 (2016)  
**Keywords:** Dialog Systems, State Tracking, Spectral Methods, Matrix Factorization, Machine Learning  
[Paper](https://arxiv.org/abs/1606.05286)

**Gated End‑to‑End Memory Networks**  
*Fei Liu, Julien Perez*  
CoRR abs/1610.04211 (2016)  
**Keywords:** Memory Networks, Gating Mechanisms, Deep Learning, Neural Networks, Machine Learning  
[Paper](https://arxiv.org/abs/1610.04211)

**A Language‑independent and Compositional Model for Personality Trait Recognition from Short Texts**  
*Fei Liu, Julien Perez, Scott Nowson*  
CoRR abs/1610.04345 (2016)  
**Keywords:** Natural Language Processing, Personality Recognition, Compositional Models, Text Analysis, Machine Learning  
[Paper](https://arxiv.org/abs/1610.04345)

### 2015

**XRCE Personal Language Analytics Engine**  
*Scott Nowson, Julien Perez, Caroline Brun, Shachar Mirkin, Claude Roux*  
CLEF (Working Notes) 2015  
**Keywords:** Natural Language Processing, Personal Analytics, Language Analysis, Text Mining, Information Retrieval  
[Paper](https://dblp.org)

**Motivating Personality‑aware Machine Translation**  
*Shachar Mirkin, Scott Nowson, Caroline Brun, Julien Perez*  
EMNLP 2015: 1102–1108  
**Keywords:** Machine Translation, Personality Analysis, Natural Language Processing, Personalization, Neural Networks  
[Paper](https://dblp.org)

**Accelerating Stochastic Gradient Descent via Online Learning to Sample**  
*Guillaume Bouchard, Théo Trouillon, Julien Perez, Adrien Gaidon*  
CoRR abs/1506.09016 (2015)  
**Keywords:** Optimization, Stochastic Gradient Descent, Online Learning, Sampling, Machine Learning  
[Paper](https://arxiv.org/abs/1506.09016)

### 2014

**QoE‑Based Server Selection for Content Distribution Networks**  
*Hai Anh Tran, Said Hoceini, Abdelhamid Mellouk, Julien Perez, Sherali Zeadally*  
IEEE Trans. Computers 63(11): 2803–2815 (2014)  
**Keywords:** Content Distribution Networks, Quality of Experience, Server Selection, Network Optimization, Distributed Systems  
[Paper](https://dblp.org)

### 2013

**A robust, adaptive and hierarchical knowledge dissemination architecture**  
*Sami Souihi, Julien Perez, Said Hoceini, Abdelhamid Mellouk*  
GLOBECOM 2013: 1489–1494  
**Keywords:** Knowledge Management, Distributed Systems, Network Architecture, Adaptive Systems, Hierarchical Systems  
[Paper](https://dblp.org)

### 2012

**User‑centric Content Distribution Network architecture**  
*Hai Anh Tran, Abdelhamid Mellouk, Said Hoceini, Julien Perez*  
ICUMT 2012: 343–350  
**Keywords:** Content Distribution Networks, User-Centric Design, Network Architecture, Distributed Systems, Multimedia  
[Paper](https://dblp.org)

### 2010

**Multi‑objective Reinforcement Learning for Responsive Grids**  
*Julien Perez, Cécile Germain‑Renaud, Balázs Kégl, Charles Loomis*  
J. Grid Comput. 8(3): 473–492 (2010)  
**Keywords:** Reinforcement Learning, Grid Computing, Multi-Objective Optimization, Distributed Systems, Resource Management  
[Paper](https://dblp.org)

**A Principled Method for Exploiting Opening Books**  
*Romaric Gaudel, Jean‑Baptiste Hoock, Julien Perez, Nataliya Sokolovska, Olivier Teytaud*  
Computers and Games 2010: 136–144  
**Keywords:** Game Theory, Monte Carlo Methods, Search Algorithms, Computer Games, Artificial Intelligence  
[Paper](https://dblp.org)

### 2009

**Grid Coevolution for Adaptive Simulations**  
*Pierre Audouard, Guillaume Chaslot, Jean‑Baptiste Hoock, Julien Perez, Arpad Rimmel, Olivier Teytaud*  
EvoWorkshops 2009: 323–332  
**Keywords:** Evolutionary Algorithms, Grid Computing, Adaptive Simulations, Coevolution, Distributed Computing  
[Paper](https://dblp.org)

**Toward autonomic grids: analyzing the job flow with affinity streaming**  
*Xiangliang Zhang, Cyril Furtlehner, Julien Perez, Cécile Germain‑Renaud, Michèle Sebag*  
KDD 2009: 987–996  
**Keywords:** Grid Computing, Data Mining, Autonomic Systems, Job Scheduling, Stream Processing  
[Paper](https://dblp.org)

### 2008

**Grid Differentiated Services: A Reinforcement Learning Approach**  
*Julien Perez, Cécile Germain‑Renaud, Balázs Kégl, Charles Loomis*  
CCGRID 2008: 287–294  
**Keywords:** Grid Computing, Reinforcement Learning, Quality of Service, Distributed Systems, Resource Allocation  
[Paper](https://dblp.org)

**Utility‑Based Reinforcement Learning for Reactive Grids**  
*Julien Perez, Cécile Germain‑Renaud, Balázs Kégl, Charles Loomis*  
ICAC 2008: 205–206  
**Keywords:** Reinforcement Learning, Grid Computing, Utility Functions, Reactive Systems, Autonomic Computing  
[Paper](https://dblp.org)

---

*For a complete list with DOI links and additional details, please visit my [Google Scholar profile](https://scholar.google.fr/citations?user=XneKjCsAAAAJ) or [DBLP page](https://dblp.org).*
